...
function getSourceCodeFromResult(result: ESLint.LintResult, config: Linter.Config): SourceCode {
  if (!result.source) throw new Error('Source code is required to apply fixes.');
  linter.verify(result.source, config, result.filePath);
  return linter.getSourceCode();
}

export type FixResult = ReturnType<typeof SourceCodeFixer.applyFixes>;

export function applyFixes(fixerOptions: FixerOptions): FixResult {
  const sourceCode = getSourceCodeFromResult(fixerOptions.result, fixerOptions.config);
